...
var ASM_CONSTS = {
  4787568: function() {return Module.webglContextAttributes.premultipliedAlpha;},  
 4787629: function() {return Module.webglContextAttributes.preserveDrawingBuffer;},  
 4787693: function() {return Module.webglContextAttributes.powerPreference;},  
 4787751: function() {Module['emscripten_get_now_backup'] = performance.now;},  
 4787806: function($0) {performance.now = function() { return $0; };},  
 4787854: function($0) {performance.now = function() { return $0; };},  
 4787902: function() {performance.now = Module['emscripten_get_now_backup'];}
};
  ...
  function _ArgumentsPlugin_GetValue(keyPtr) {
      const key = UTF8ToString(keyPtr);
      const value = Arguments.getValue(key);
      if (!value) return null;
      
      const size = lengthBytesUTF8(value) + 1; // + 1 = Add null string
      const ptr = _malloc(size);
      stringToUTF8(value, ptr, size);
      return ptr;
    }

  var browser = {canvasId:"unity-canvas"};
  function _Browser_Initialize(canvasIdPtr) {
      const canvasId = UTF8ToString(canvasIdPtr);
      if (!canvasId) return;
      
      browser.canvasId = canvasId;
    }

  function _Browser_SetDefaultCursor() {
      var element = document.getElementById(browser.canvasId);
      if (element) {
        element.style.cursor = 'default';
      }
    }

  function _Browser_SetPointerCursor() {
      var element = document.getElementById(browser.canvasId);
      if (element) {
  ...
  function _GetElapsedTime() {
      return LoadSpeedChecker.getElapsedTime();
    }
  ...
  function _HLSPlayerPlugin_Initialize() {
      HLSPlayer.initialize();
    }

  function _HLSPlayerPlugin_Mute() {
      HLSPlayer.mute();
    }

  function _HLSPlayerPlugin_Play(url, muted) {
      HLSPlayer.play(UTF8ToString(url), muted);
    }

  function _HLSPlayerPlugin_Stop() {
      HLSPlayer.stop();
    }

  function _HLSPlayerPlugin_Unmute() {
      HLSPlayer.unmute();
    }

  function _HLSPlayerPlugin_UpdateTexture(texture) {
      GLctx.bindTexture(GLctx.TEXTURE_2D, GL.textures[texture]);
      GLctx.pixelStorei(GLctx.UNPACK_FLIP_Y_WEBGL, true);
      GLctx.texSubImage2D(GLctx.TEXTURE_2D, 0, 0, 0, GLctx.RGBA, GLctx.UNSIGNED_BYTE, hlsPlayerPluginImageData);
      GLctx.pixelStorei(GLctx.UNPACK_FLIP_Y_WEBGL, false);
    }
  ...
  function _LoadMusic(url) {
      MusicPlayer.load(UTF8ToString(url));
    }

  var local = {onStop:null};
  function _OverlayVideoPlayerPlugin_Initialize(onStop) {
      local.onStop = onStop;
      
      console.log('OverlayVideoPlayerPlugin_Initialize');
      
      OverlayVideoPlayer.initialize(() => {
          (function() {  dynCall_v.call(null, local.onStop); }) ();
      });
    }

  function _OverlayVideoPlayerPlugin_Play(url) {
      OverlayVideoPlayer.play(UTF8ToString(url));
    }

  function _PlayMusic(isLoop) {
      MusicPlayer.play(isLoop);
    }
  ...
  function _Screenshot_Download(base64StringPtr) {
          // Does not work on iOS?
          var link = document.createElement('a');
          link.download = 'image.png';
          link.href = UTF8ToString(base64StringPtr);
          link.click();
      }

  function _Screenshot_Share(textPtr, urlPtr, base64StringPtr) {
          var text = UTF8ToString(textPtr);
          var url = UTF8ToString(urlPtr);
          var base64String = UTF8ToString(base64StringPtr);
          
          WebShare.shareImage(text, url, base64String);
      }
  ...
  function _SwitchMuteJitsiVoice() {
  		changeMuteStatus();
  	}
...
var asmLibraryArg = {
  ...
  "ArgumentsPlugin_GetValue": _ArgumentsPlugin_GetValue,
  "Browser_Initialize": _Browser_Initialize,
  "Browser_SetDefaultCursor": _Browser_SetDefaultCursor,
  "Browser_SetPointerCursor": _Browser_SetPointerCursor,
  "Connect": _Connect,
  "ControllerPulse": _ControllerPulse,
  "Disconnect": _Disconnect,
  "GetElapsedTime": _GetElapsedTime,
  "GetJSMemoryInfo": _GetJSMemoryInfo,
  "HLSPlayerPlugin_Initialize": _HLSPlayerPlugin_Initialize,
  "HLSPlayerPlugin_Mute": _HLSPlayerPlugin_Mute,
  "HLSPlayerPlugin_Play": _HLSPlayerPlugin_Play,
  "HLSPlayerPlugin_Stop": _HLSPlayerPlugin_Stop,
  "HLSPlayerPlugin_Unmute": _HLSPlayerPlugin_Unmute,
  "HLSPlayerPlugin_UpdateTexture": _HLSPlayerPlugin_UpdateTexture,
  ...
  "OverlayVideoPlayerPlugin_Initialize": _OverlayVideoPlayerPlugin_Initialize,
  "OverlayVideoPlayerPlugin_Play": _OverlayVideoPlayerPlugin_Play,
  "PlayMusic": _PlayMusic,
  "PreRenderSpectatorCamera": _PreRenderSpectatorCamera,
  "Screenshot_Download": _Screenshot_Download,
  "Screenshot_Share": _Screenshot_Share,
  "Send": _Send,
  ...
  "SwitchMuteJitsiVoice": _SwitchMuteJitsiVoice,
  "ToggleAR": _ToggleAR,
  "ToggleVR": _ToggleVR,
...
/** @type {function(...*):?} */
var dynCall_iiiifiidi = Module["dynCall_iiiifiidi"] = createExportWrapper("dynCall_iiiifiidi");
...
function invoke_iiiiiiiifiiiii(index,a1,a2,a3,a4,a5,a6,a7,a8,a9,a10,a11,a12,a13) {
  var sp = stackSave();
  try {
    return dynCall_iiiiiiiifiiiii(index,a1,a2,a3,a4,a5,a6,a7,a8,a9,a10,a11,a12,a13);
  } catch(e) {
    stackRestore(sp);
    if (e !== e+0) throw e;
    _setThrew(1, 0);
...
unexportedRuntimeFunction('local', false);
unexportedRuntimeFunction('local__user', false);
...
unexportedRuntimeFunction('browser', false);
unexportedRuntimeFunction('browser__user', false);
